...
<?php
chdir("Files");
$file = 'Selected Publications.htm';
...
if ( strcmp($data,$dummy) != 0) {
	...
	{ 
		...
		if(strstr($eachDataArray,'Works in Process'))
		{
			echo '<ul>';
			$i = 1;
			continue;
		...
		if($i == 0)
			continue;
		if(strstr($eachDataArray,'Select listing of Professional and Academic Panel Presentations'))
			break;
		...
		{
			if(! strstr($eachDataArrayTrimmed, 'Research text book currently under review'))
				printf($format, $html); 
			else
				printf($sep_format, $html);
		}
	}
